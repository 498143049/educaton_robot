...
int32_t BMX055_ReadMag(bmx055_t *pbmx055) {
    ...
    pbmx055->mx = -pbmx055->raw_mag.datay * 0.3f;
    pbmx055->my = pbmx055->raw_mag.datax * 0.3f;
    pbmx055->mz = pbmx055->raw_mag.dataz * 0.3f;
#if CALIBRATION__MAGN_USE_EXTENDED
    float magtemp[3], magtemp2[3];
...
int32_t BMX055_ReadAccGyro(bmx055_t *pbmx055) {
    ...
    // 单位转换
    pbmx055->ax = -pbmx055->raw_acc.y / 256.0f;
    pbmx055->ay = pbmx055->raw_acc.x / 256.0f;
    pbmx055->az = pbmx055->raw_acc.z / 256.0f;
    pbmx055->gx = -pbmx055->raw_gyro.datay / 65.6f;
    pbmx055->gy = pbmx055->raw_gyro.datax / 65.6f;
    pbmx055->gz = pbmx055->raw_gyro.dataz / 65.6f;
    // 误差校准
...
{
    s32 iError;
    if (I2C_WriteBurst(dev_addr << 1, reg_addr, reg_data, cnt) == I2C_SUCCESS) {
        iError = 0;
    } else {
        iError = -1;
    }
...
s8 BMX055_I2C_bus_burst_read(u8 dev_addr, u8 reg_addr, u8 *reg_data, u32 cnt) {
    s32 iError;
    if (I2C_ReadBurst(dev_addr << 1, reg_addr, reg_data, cnt) == I2C_SUCCESS) {
        iError = 0;
    } else {
        iError = -1;
    }
